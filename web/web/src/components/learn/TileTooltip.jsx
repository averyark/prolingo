...
export const TileTooltip = ({
  ...
  const ATTEMPT_ENERGY_COST = 5;
  const PRACTICE_ENERGY_COST = 2;
  // ACTIVE => full attempt, other non-locked => practice
  ...
  const tryStart = (testId) => {
    if (typeof energy !== "number" || energy < requiredEnergy) {
      toast.error("Not enough energy to start this test.");
      return;
    }
    closeTooltip?.();
    ...
    >
      ...
      >
        ...
        {status === "ACTIVE" ? (
          <button
            type="button"
            onClick={() => tryStart(selectedTileInfo?.test_id)}
            className={[

...
class ClientSubmitUserTestView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # include score_count and whether the attempt passed (if test has passing_score)
        try:
            score_count = int(getattr(result.user_test, "score_count", 0))
        except Exception:
            score_count = None
        response_payload["score_count"] = score_count
        try:
            passing_score = getattr(result.user_test.test, "passing_score", None)
            if passing_score is None:
                response_payload["passed"] = None
                response_payload["passing_score"] = None
            else:
                response_payload["passing_score"] = passing_score
                response_payload["passed"] = bool(score_count is not None and score_count >= int(passing_score))
        except Exception:
            # don't block response if relationship missing
            response_payload.setdefault("passed", None)
            response_payload.setdefault("passing_score", None)
